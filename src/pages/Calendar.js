...
export default function Calendar() {
  ...
  const dayMap = useMemo(() => {
    ...
    trades.forEach(t => {
      ...
      if (t.isWithdrawal) {
        // Track withdrawals separately — never mix into P&L
  ...
  return (
    <div>
      ...
      <div className="page-body" style={{display:'grid',gridTemplateColumns:'1fr 260px',gap:16,alignItems:'start'}}>
        <div className="card" style={{padding:'16px'}}>
          {/* Column headers */}
          <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr) 110px',gap:4,marginBottom:4}}>
          ...
          {Array.from({length:totalWeeks},(_,wi)=>{
            ...
            return (
              <div key={wi} style={{display:'grid',gridTemplateColumns:'repeat(7,1fr) 110px',gap:4,marginBottom:4}}>
                {Array.from({length:7},(_,di)=>{
                  const d    = dn(wi,di);
                  if (d<1||d>daysInMonth) return <div key={di} style={{height:68}}/>;
                  const date  = ds(d);
                  ...
                  return (
                    <div key={di} onClick={()=>setSelected(isSel?null:date)}
                      style={{
                        height:68, borderRadius:6, padding:'6px 7px', boxSizing:'border-box',
                      ...
                      {/* Withdrawal badge — bottom of cell, distinct style */}
                      {wd && (
                        <div style={{
                          marginTop: cell?2:0,
                ...
                {/* Weekly cell */}
                <div style={{
                  height:68, borderRadius:6, padding:'7px 10px', boxSizing:'border-box',
