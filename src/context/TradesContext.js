...
export function TradesProvider({ children }) {
  ...
  // Supabase client ref — recreated when config changes
  const sbRef = useRef(getSupabaseClient());

  // On mount: load from Supabase if configured, overwriting local cache
  useEffect(() => {
    ...
    loadAllFromSupabase(client).then(data => {
      ...
      // Restore password hash from cloud so password works on new devices
  ...
  // Helper: save to localStorage + cloud
  ...
  const refreshSupabaseClient = useCallback(() => {
    ...
    if (!sbRef.current) { setCloudStatus('idle'); return; }
    setCloudStatus('syncing');
